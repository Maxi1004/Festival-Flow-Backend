"""Backfill crew_members.talent_uid from the legacy talent_user_id field."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Backfill missing crew_members.talent_uid values from talent_user_id.",
    )
    mode = parser.add_mutually_exclusive_group(required=True)
    mode.add_argument("--dry-run", action="store_true", help="Report eligible documents without writing changes.")
    mode.add_argument("--apply", action="store_true", help="Apply updates in Firestore batches.")
...
def main() -> int:
    ...
    try:
        eligible_docs, scanned, skipped_existing, skipped_missing_legacy = get_eligible_crew_members()
    except Exception as error:
